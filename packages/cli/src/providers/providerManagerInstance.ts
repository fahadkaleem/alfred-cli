...
import type {
  Config} from '@alfred/alfred-cli-core';
import {
  ProviderManager,
  OpenAIProvider,
  ...
  if (!providerManagerInstance) {
    ...
    if (!loadedSettings) {
      ...
      loadedSettings = userSettings
        ? new LoadedSettings(
            { path: '', settings: {}, originalSettings: {} }, // system
            { path: '', settings: {}, originalSettings: {} }, // systemDefaults
            {
              path: USER_SETTINGS_PATH,
              settings: userSettings,
              originalSettings: userSettings,
            }, // user
            { path: '', settings: {}, originalSettings: {} }, // workspace
            true, // isTrusted
            new Set(), // migratedInMemorScopes
          )
        : undefined;
    }
    ...
    const openaiProviderConfig = {
      enableTextToolCallParsing: (
        settingsData as { enableTextToolCallParsing?: boolean }
      ).enableTextToolCallParsing,
      textToolCallModels: (settingsData as { textToolCallModels?: string[] })
        .textToolCallModels,
      providerToolFormatOverrides: (
        settingsData as { providerToolFormatOverrides?: Record<string, string> }
      ).providerToolFormatOverrides,
      openaiResponsesEnabled: (
        settingsData as { openaiResponsesEnabled?: boolean }
      ).openaiResponsesEnabled,
