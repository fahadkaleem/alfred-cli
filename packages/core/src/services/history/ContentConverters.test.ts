...
describe('ContentConverters - Tool ID Normalization', () => {
  describe('toIContent - Converting TO History Format', () => {
    it('should preserve original IDs and normalize prefix to history format', () => {
      // CRITICAL BUG FIX TEST: OpenAI uses short alphanumeric IDs like '692a5fddc'
        ...
        parts: [
          {
            functionCall: {
              name: 'glob',
              args: { pattern: '**/*.ts' },
              id: '692a5fddc', // OpenAI style ID - MUST BE IGNORED
            },
          },
        ],
      };
    ...
    it('should normalize OpenAI tool IDs to history format', () => {
      // OpenAI uses short alphanumeric IDs like '692a5fddc'
      const geminiContent: Content = {
        role: 'model',
        parts: [
          {
  ...
  describe('Real-world Provider Switching Scenario', () => {
    it('should handle the exact error scenario from debug log with prefix normalization', () => {
      ...
      // OpenAI tool ID '692a5fddc' caused Anthropic 400 error
      // The fix requires using callback system to maintain ID consistency

      // Step 1: Assistant makes tool call with OpenAI ID
        ...
        parts: [
          ...
          },
          {
            functionCall: {
              name: 'glob',
              args: { pattern: '**/*.ts' },
              id: '692a5fddc', // The problematic OpenAI ID - MUST BE IGNORED
            },
          },
        ],
      };
        ...
        parts: [
          ...
          },
          {
            functionCall: {
              name: 'glob',
              args: { pattern: '**/*.ts' },
              id: '692a5fddc', // The problematic OpenAI ID
            },
          },
        ],
      };
...
describe('ContentConverters - History ID Conversion for Gemini', () => {
  ...
  describe('converting Gemini Content to IContent', () => {
    it('should generate history IDs for tool calls without IDs', () => {
      ...
      const toolCallBlock = iContent.blocks[0] as ToolCallBlock;
      expect(toolCallBlock.id).toBeDefined();
      expect(toolCallBlock.id).not.toBe('');
    ...
    it('should normalize tool call IDs even when provided (for provider switching fix)', () => {
      ...
      expect(toolCallBlock.id).not.toBe('some_id');
    });
    ...
    it('should handle tool responses maintaining position', () => {
      ...
      expect(toolResponseBlock.callId).toMatch(/^hist_tool_/); // Generates history ID when no position matching
      expect(toolResponseBlock.toolName).toBe('get_weather');
    });
    ...
    it('should maintain order for multiple tool calls', () => {
      ...
      expect((iContent.blocks[0] as ToolCallBlock).name).toBe('tool_a');
      expect((iContent.blocks[1] as ToolCallBlock).name).toBe('tool_b');
      expect((iContent.blocks[2] as ToolCallBlock).name).toBe('tool_c');
  ...
  describe('round-trip conversion', () => {
    it('should maintain tool relationships through round-trip conversion', () => {
      ...
      expect((roundTripContent[0].blocks[0] as ToolCallBlock).name).toBe(
        'search',
      );
      expect((roundTripContent[0].blocks[1] as ToolCallBlock).name).toBe(
        'write',
      );

      // Tool responses preserved
      expect(
        (roundTripContent[1].blocks[0] as ToolResponseBlock).toolName,
      ).toBe('search');
      expect(
        (roundTripContent[2].blocks[0] as ToolResponseBlock).toolName,
      ).toBe('write');

      // IDs normalized to history format (new ones generated since old behavior changed)
      expect((roundTripContent[0].blocks[0] as ToolCallBlock).id).toMatch(
        /^hist_tool_/,
      );
      expect((roundTripContent[0].blocks[1] as ToolCallBlock).id).toMatch(
        /^hist_tool_/,
      );
  ...
  describe('position-based matching for Gemini', () => {
    it('should support position-based tool response matching', () => {
      ...
      expect((iContentCalls.blocks[0] as ToolCallBlock).id).toBeDefined();
      expect((iContentCalls.blocks[1] as ToolCallBlock).id).toBeDefined();
      expect((iContentCalls.blocks[2] as ToolCallBlock).id).toBeDefined();

      // Responses generate history IDs when no position matching callback
      expect(
        (iContentResponses[0].blocks[0] as ToolResponseBlock).callId,
      ).toMatch(/^hist_tool_/);
      expect(
        (iContentResponses[1].blocks[0] as ToolResponseBlock).callId,
      ).toMatch(/^hist_tool_/);
      expect(
        (iContentResponses[2].blocks[0] as ToolResponseBlock).callId,
      ).toMatch(/^hist_tool_/);

      // But they maintain order/position for matching
      expect(
        (iContentResponses[0].blocks[0] as ToolResponseBlock).toolName,
      ).toBe('first');
      expect(
        (iContentResponses[1].blocks[0] as ToolResponseBlock).toolName,
      ).toBe('second');
      expect(
        (iContentResponses[2].blocks[0] as ToolResponseBlock).toolName,
      ).toBe('third');
    });
  });
  ...
  describe('ID Normalization Architecture - NEW FAILING TESTS', () => {
    ...
    describe('Position-based matching with callbacks', () => {
      it('should match multiple tool responses by position order', () => {
        ...
        expect((results[0].blocks[0] as ToolResponseBlock).callId).toBe(
          'hist_tool_1',
        );
        expect((results[1].blocks[0] as ToolResponseBlock).callId).toBe(
          'hist_tool_2',
        );
        expect((results[2].blocks[0] as ToolResponseBlock).callId).toBe(
          'hist_tool_3',
        );
      ...
      it('should handle tool responses when no unmatched calls exist', () => {
        ...
        const getNextUnmatchedCall = () =>
          undefined as unknown as { historyId: string; toolName?: string }; // No unmatched calls
    ...
    describe('Integration with HistoryService callbacks', () => {
      it('should work with HistoryService generateHistoryId callback', () => {
        ...
        expect((result.blocks[0] as ToolCallBlock).id).toMatch(/^hist_tool_/);
        expect((result.blocks[1] as ToolCallBlock).id).toMatch(/^hist_tool_/);
        expect((result.blocks[0] as ToolCallBlock).id).not.toBe(
          (result.blocks[1] as ToolCallBlock).id,
        );
      });
