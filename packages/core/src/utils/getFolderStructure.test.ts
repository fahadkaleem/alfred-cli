...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile('.alfred', 'config.yaml');
      await createTestFile('.alfred', 'logs.json');
  ...
  describe('with alfredignore', () => {
    it('should ignore alfredignore files by default', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.alfredignore'),
        'ignored.txt\nnode_modules/\n.alfred/\n!/.alfred/config.yaml',
      );
      await createTestFile('file1.txt');
      await createTestFile('node_modules', 'some-package', 'index.js');
    ...
    it('should not ignore files if respectAlfredIgnore is false', async () => {
      await fsPromises.writeFile(
        nodePath.join(testRootDir, '.alfredignore'),
        'ignored.txt\nnode_modules/\n.alfred/\n!/.alfred/config.yaml',
      );
