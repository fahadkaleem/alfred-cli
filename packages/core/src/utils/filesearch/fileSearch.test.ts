...
import { createTmpDir, cleanupTmpDir } from '@alfred/alfred-cli-test-utils';
...
describe('FileSearch', () => {
  ...
  it('should use .alfredignore rules', async () => {
    ...
    expect(results).toEqual(['src/', '.alfredignore', 'src/not-ignored.js']);
  });

  it('should combine .gitignore and .alfredignore rules', async () => {
    tmpDir = await createTmpDir({
      '.gitignore': 'dist/',
      '.alfredignore': 'build/',
