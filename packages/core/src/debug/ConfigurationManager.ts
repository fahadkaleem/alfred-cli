...
export class ConfigurationManager {
  ...
  private constructor() {
    this.defaultConfig = {
      ...
      namespaces: [],
      level: 'info',
      output: { target: 'file', directory: `~/${ALFRED_DIR}/debug` },
      lazyEvaluation: true,
  ...
  private loadUserConfig(): void {
    try {
      ...
      const configPath = path.join(homeDir, ALFRED_DIR, 'settings.json');
  ...
  private loadProjectConfig(): void {
    try {
      ...
      const configPath = path.join(cwd, ALFRED_DIR, 'config.json');
      if (fs.existsSync(configPath)) {
        try {
          const content = fs.readFileSync(configPath, 'utf8');
          const parsed = JSON.parse(content);
  ...
  persistEphemeralConfig(): void {
    ...
    const userConfigPath = path.join(os.homedir(), ALFRED_DIR, 'settings.json');
