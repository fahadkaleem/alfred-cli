...
describe('DebugLogger', () => {
  ...
  it('should create logger with namespace @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    const logger = new DebugLogger('alfred:test');
    expect(logger).toBeDefined();
    expect(logger.namespace).toBe('alfred:test');
  });
   ...
   * @given namespace 'alfred:openai:tools'
   * @when pattern 'alfred:openai:*' configured
   * @then Logger is enabled
   */
  it('should match wildcard namespaces @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    const logger = new DebugLogger('alfred:openai:tools');
    logger.configManager.setEphemeralConfig({
      enabled: true,
      namespaces: ['alfred:openai:*'],
    });
   ...
   * @given namespace 'alfred:core:utils:parser'
   * @when pattern 'alfred:core:*' configured
   * @then Logger matches pattern
   */
  it('should match complex namespace patterns @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    const logger = new DebugLogger('alfred:core:utils:parser');
    logger.configManager.setEphemeralConfig({
      enabled: true,
      namespaces: ['alfred:core:*'],
    });
   ...
   * @given namespace 'alfred:test'
   * @when exact pattern 'alfred:test' configured
   * @then Logger is enabled
   */
  ...
  it('should match multiple namespace patterns @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    const logger = new DebugLogger('alfred:openai');
    logger.configManager.setEphemeralConfig({
      enabled: true,
      namespaces: ['alfred:core:*', 'alfred:openai', 'alfred:cli:*'],
    });
   ...
   * @given namespace 'alfred:test'
   * @when pattern 'alfred:other:*' configured
  ...
  it('should not match unrelated namespace patterns @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    ...
    logger.configManager.setEphemeralConfig({
      enabled: true,
      namespaces: ['alfred:other:*', 'different:namespace'],
    });
  ...
  it('should handle any namespace string @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    fc.assert(
      fc.property(fc.string(), (namespace: string) => {
  ...
  it('should handle any message type when disabled @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    fc.assert(
      fc.property(fc.anything(), (message: unknown) => {
  ...
  it('should consistently match namespace patterns @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    fc.assert(
      fc.property(
        fc.string().filter((s: string) => s.length > 0),
        fc.string().filter((s: string) => s.length > 0),
  ...
  it('should handle multiple namespace patterns correctly @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    fc.assert(
      fc.property(
        ...
        (namespace: string, patterns: string[]) => {
          const logger = new DebugLogger(namespace);
          logger.configManager.setEphemeralConfig({
            enabled: true,
    ...
    fc.assert(
      fc.property(
        ...
        (messages: string[]) => {
          ...
          messages.forEach((msg: string) => logger.log(msg));
  ...
  it('should handle function evaluation consistently @plan:PLAN-20250120-DEBUGLOGGING.P04', () => {
    fc.assert(
      fc.property(
        fc.boolean(),
        fc.string(),
        (enabled: boolean, returnValue: string) => {
          const logger = new DebugLogger('test');
  ...
  describe('enhanced pattern matching', () => {
     ...
     * @given namespace 'alfred:openai:provider'
     * @when pattern 'alfred:*:provider' configured
     * @then Logger is enabled
     */
    it('should match wildcards in middle of pattern', () => {
      const logger = new DebugLogger('alfred:openai:provider');
      logger.configManager.setEphemeralConfig({
        enabled: true,
        namespaces: ['alfred:*:provider'],
      });
     ...
     * @given namespace 'alfred:openai:streaming:chunk'
     * @when pattern 'alfred:*:*:chunk' configured
     * @then Logger is enabled
     */
    it('should match multiple wildcards in pattern', () => {
      const logger = new DebugLogger('alfred:openai:streaming:chunk');
      logger.configManager.setEphemeralConfig({
        enabled: true,
        namespaces: ['alfred:*:*:chunk'],
      });
     ...
     * @given namespace 'alfred:openai:provider'
     * @when pattern 'alfred:openai' configured (no wildcard)
    ...
    it('should correctly filter with mixed wildcard patterns', () => {
      const pattern = 'alfred:*:provider';

      const openaiLogger = new DebugLogger('alfred:openai:provider');
      openaiLogger.configManager.setEphemeralConfig({
        enabled: true,
        namespaces: [pattern],
      });

      const anthropicLogger = new DebugLogger('alfred:anthropic:provider');
      anthropicLogger.configManager.setEphemeralConfig({
        enabled: true,
        namespaces: [pattern],
      });

      const streamingLogger = new DebugLogger('alfred:openai:streaming');
