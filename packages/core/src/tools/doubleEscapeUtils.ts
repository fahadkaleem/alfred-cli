 ...
 */
export function shouldUseDoubleEscapeHandling(_toolFormat: string): boolean {
  // No formats currently need double-escape handling
  return false;
}
  ...
  try {
    ...
    if (typeof parsed === 'string') {
      ...
      try {
        ...
        logger['debug'](
          () => `JSON parameters are single-stringified (normal)`,
        );
...
export function processToolParameters(
  ...
  // Apply double-escape detection and fixing
