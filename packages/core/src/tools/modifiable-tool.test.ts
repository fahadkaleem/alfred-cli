...
describe('modifyWithEditor', () => {
  ...
  afterEach(async () => {
    ...
    const diffDir = path.join(os.tmpdir(), 'alfred-cli-tool-modify-diffs');
  ...
  it('should create temp files with correct naming with extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/alfred-cli-modify-test-file-old-\d+\.txt$/);
    expect(newFilePath).toMatch(/alfred-cli-modify-test-file-new-\d+\.txt$/);
  ...
  it('should create temp files with correct naming without extension', async () => {
    ...
    expect(mockOpenDiff).toHaveBeenCalledOnce();
    const [oldFilePath, newFilePath] = mockOpenDiff.mock.calls[0];
    expect(oldFilePath).toMatch(/alfred-cli-modify-test-file-old-\d+$/);
    expect(newFilePath).toMatch(/alfred-cli-modify-test-file-new-\d+$/);
