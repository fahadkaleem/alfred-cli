...
export class ToolFormatter implements IToolFormatter {
  ...
  convertGeminiToFormat(
    ...
    // For OpenAI-compatible formats (openai, deepseek), use the OpenAI conversion
  ...
  convertGeminiSchemaToStandard(schema: unknown): unknown {
    ...
    if (newSchema['items']) {
      if (Array.isArray(newSchema['items'])) {
        newSchema['items'] = (newSchema['items'] as unknown[]).map((item) =>
          this.convertGeminiSchemaToStandard(item),
        );
      } else {
        newSchema['items'] = this.convertGeminiSchemaToStandard(
          newSchema['items'],
        );
      }
    }
    ...
    if (newSchema['enum'] && Array.isArray(newSchema['enum'])) {
      newSchema['enum'] = (newSchema['enum'] as unknown[]).map((v) =>
        String(v),
      );
    }
    ...
    if (newSchema['minLength'] && typeof newSchema['minLength'] === 'string') {
      const minLengthNum = parseInt(newSchema['minLength'] as string, 10);
    ...
    if (newSchema['maxLength'] && typeof newSchema['maxLength'] === 'string') {
      const maxLengthNum = parseInt(newSchema['maxLength'] as string, 10);
