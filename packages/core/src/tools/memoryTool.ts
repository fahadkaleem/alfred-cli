...
export const ALFRED_CONFIG_DIR = '.alfred';
export const DEFAULT_CONTEXT_FILENAME = 'ALFRED.md';
export const MEMORY_SECTION_HEADER = '## Alfred Added Memories';

// This variable will hold the currently configured filename for ALFRED.md context files.
// It defaults to DEFAULT_CONTEXT_FILENAME but can be overridden by setAlfredMdFilename.
let currentAlfredMdFilename: string | string[] = DEFAULT_CONTEXT_FILENAME;

export function setAlfredMdFilename(newFilename: string | string[]): void {
  if (Array.isArray(newFilename)) {
    if (newFilename.length > 0) {
      currentAlfredMdFilename = newFilename.map((name) => name.trim());
    }
  } else if (newFilename && newFilename.trim() !== '') {
    currentAlfredMdFilename = newFilename.trim();
  }
}

export function getCurrentAlfredMdFilename(): string {
  if (Array.isArray(currentAlfredMdFilename)) {
    return currentAlfredMdFilename[0];
  }
  return currentAlfredMdFilename;
}

export function getAllAlfredMdFilenames(): string[] {
  if (Array.isArray(currentAlfredMdFilename)) {
    return currentAlfredMdFilename;
  }
  return [currentAlfredMdFilename];
...
function getGlobalMemoryFilePath(): string {
  return path.join(Storage.getGlobalAlfredDir(), getCurrentAlfredMdFilename());
}
