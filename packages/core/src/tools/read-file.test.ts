...
describe('ReadFileTool', () => {
  ...
  describe('execute', () => {
    ...
    describe('with .alfredignore', () => {
      ...
      it('should throw error if path is ignored by a .alfredignore pattern', async () => {
        const ignoredFilePath = path.join(tempRootDir, 'foo.bar');
        await fsp.writeFile(ignoredFilePath, 'content', 'utf-8');
        const params: ReadFileToolParams = {
          absolute_path: ignoredFilePath,
        };
        const expectedError = `File path '${ignoredFilePath}' is ignored by .alfredignore pattern(s).`;
