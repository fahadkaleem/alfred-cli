...
const MEMORY_SECTION_HEADER = '## Alfred Added Memories';
...
describe('MemoryTool', () => {
  ...
  afterEach(() => {
    vi.restoreAllMocks();
    // Reset ALFRED_MD_FILENAME to its original value after each test
    setAlfredMdFilename(DEFAULT_CONTEXT_FILENAME);
  });

  describe('setAlfredMdFilename', () => {
    it('should update currentAlfredMdFilename when a valid new name is provided', () => {
      const newName = 'CUSTOM_CONTEXT.md';
      setAlfredMdFilename(newName);
      expect(getCurrentAlfredMdFilename()).toBe(newName);
    });

    it('should not update currentAlfredMdFilename if the new name is empty or whitespace', () => {
      const initialName = getCurrentAlfredMdFilename(); // Get current before trying to change
      setAlfredMdFilename('  ');
      expect(getCurrentAlfredMdFilename()).toBe(initialName);
    ...
    it('should handle an array of filenames', () => {
      ...
      setAlfredMdFilename(newNames);
      expect(getCurrentAlfredMdFilename()).toBe('CUSTOM_CONTEXT.md');
      expect(getAllAlfredMdFilenames()).toEqual(newNames);
    });
  });
  ...
  describe('execute (instance method)', () => {
    ...
    it('should call performAddMemoryEntry with correct parameters and return success', async () => {
      ...
      // Use getCurrentAlfredMdFilename for the default expectation before any setAlfredMdFilename calls in a test
      const expectedFilePath = path.join(
        os.homedir(),
        '.alfred',
        getCurrentAlfredMdFilename(), // This will be DEFAULT_CONTEXT_FILENAME unless changed by a test
      );
  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', '.alfred', 'ALFRED.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(path.join('mock', 'home', '.alfred'));
        expect(result.fileName).toContain('ALFRED.md');
        expect(result.fileDiff).toContain('Index: ALFRED.md');
        expect(result.fileDiff).toContain('+## Alfred Added Memories');
        expect(result.fileDiff).toContain('+- Test fact');
        expect(result.originalContent).toBe('');
        expect(result.newContent).toContain('## Alfred Added Memories');
