...
export interface AuthPrecedenceConfig {
  // Provider name for provider-specific key lookups
  providerName?: string;
...
export class AuthPrecedenceResolver {
  ...
  async resolveAuthentication(): Promise<string | null> {
    ...
    // 1. Check provider-specific /key command key (highest priority)
    if (this.config.providerName) {
      const providerSpecificKey = `${this.config.providerName}-auth-key`;
      const providerAuthKey = settingsService.get(providerSpecificKey);
      if (
        providerAuthKey &&
    ...
    // 2. Check global /key command key (for backward compatibility)
