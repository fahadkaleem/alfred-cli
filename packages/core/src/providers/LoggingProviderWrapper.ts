...
import type { IProvider, IModel, ITool } from './IProvider.js';
...
export class LoggingProviderWrapper implements IProvider {
    ...
    try {
      ...
      if (typeof response === 'string') {
        ...
        if (obj['usage'] && typeof obj['usage'] === 'object') {
          ...
          cached_content_token_count =
            Number(usage['cached_content_tokens']) || 0;
  ...
  private accumulateTokenUsage(tokenCounts: {
    ...
    // Token tracking is handled internally by the performance tracker
    // No need to call external provider manager
    console.debug(
      `[TokenTracking] Tracked ${usage.input + usage.output + usage.cache + usage.tool + usage.thought} tokens for provider ${this.wrapped.name}`,
    );
  }
