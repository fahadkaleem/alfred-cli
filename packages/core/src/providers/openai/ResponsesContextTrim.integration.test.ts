...
describe.skip('ResponsesContextTrim Integration', () => {
  ...
  it('should warn when approaching context limit', async () => {
    ...
    const messages: OpenAIMessage[] = [{ role: 'user', content: largeContent }];
    ...
    const generator = await (
      provider as unknown as {
        ...
        ) => Promise<AsyncIterableIterator<OpenAIMessage>>;
      }
  ...
  it('should track token accumulation across multiple calls', async () => {
    ...
    const messages1Collected: OpenAIMessage[] = [];
    ...
    const messages2Collected: OpenAIMessage[] = [];
    ...
    // Note: estimateContextUsage method was removed from OpenAIResponsesProvider
    // Skipping context estimation verification as the method no longer exists
  });
});
