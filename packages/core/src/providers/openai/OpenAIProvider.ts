...
export class OpenAIProvider extends BaseProvider implements IProvider {
  ...
  constructor(
    ...
    // new DebugLogger('alfred:core:toolformatter'), // TODO: Fix ToolFormatter constructor

    // Setup debug logger
    this.logger = new DebugLogger('alfred:provider:openai');
  }
  ...
  private createHttpAgents():
    ...
    if (socketNoDelay) {
      const httpAgentAny = httpAgent as Record<string, unknown>;
      const originalCreateConnection = httpAgentAny.createConnection as (
        options: Record<string, unknown>,
        callback: (err?: Error, socket?: net.Socket) => void,
      ) => net.Socket;
      httpAgentAny.createConnection = function (
        options: Record<string, unknown>,
        callback: (err?: Error, socket?: net.Socket) => void,
      ) {
      ...
      const httpsAgentAny = httpsAgent as Record<string, unknown>;
      const originalHttpsCreateConnection = httpsAgentAny.createConnection as (
        options: Record<string, unknown>,
        callback: (err?: Error, socket?: net.Socket) => void,
      ) => net.Socket;
      httpsAgentAny.createConnection = function (
        options: Record<string, unknown>,
        callback: (err?: Error, socket?: net.Socket) => void,
      ) {
  ...
  private async *generateChatCompletionImpl(
    ...
    const userMemory = this.globalConfig?.getUserMemory
      ...
      : '';
    const systemPrompt = await getCoreSystemPromptAsync(userMemory);
