...
// OpenAI-specific message format for internal API testing
...
interface OpenAIProviderPrivate {
  ...
  ) => AsyncIterableIterator<OpenAIMessage>;
}
...
describe.skip('OpenAIProvider Responses Integration', () => {
  ...
  it('should make a successful streaming request', async () => {
    ...
    const messages: IContent[] = [
      { speaker: 'human', blocks: [{ type: 'text', text: 'Say hello' }] },
    ];

    const result = await provider.generateChatCompletion(messages);
    const collectedMessages: IContent[] = [];
    ...
    // Check for text blocks containing the expected content
    const hasHello = collectedMessages.some((m) =>
      m.blocks.some((b) => b.type === 'text' && b.text === 'Hello'),
    );
    const hasWorld = collectedMessages.some((m) =>
      m.blocks.some((b) => b.type === 'text' && b.text === ' world'),
    );
  ...
  it('should cache and reuse conversations', async () => {
    ...
    const collected1: OpenAIMessage[] = [];
    ...
    const collected2: OpenAIMessage[] = [];
  ...
  it('should handle tool calls correctly', async () => {
    ...
    // Find the message with tool call blocks
    const toolCallMessage = collectedMessages.find((m) =>
      m.blocks.some((b) => b.type === 'tool_call'),
    );
    expect(toolCallMessage).toBeDefined();
    const toolCallBlocks = toolCallMessage?.blocks.filter(
      (b) => b.type === 'tool_call',
    );
    expect(toolCallBlocks).toHaveLength(1);
    if (toolCallBlocks?.[0]?.type === 'tool_call') {
      expect(toolCallBlocks[0].id).toBe('call_123');
      expect(toolCallBlocks[0].name).toBe('search');
      expect(toolCallBlocks[0].parameters).toEqual({ query: 'weather' });
    }
  });
  ...
  it('should handle API errors correctly', async () => {
    ...
    const messages: IContent[] = [
      { speaker: 'human', blocks: [{ type: 'text', text: 'Test error' }] },
    ];
  ...
  it('should handle non-streaming responses', async () => {
    ...
    const collectedMessages: OpenAIMessage[] = [];
  ...
  it('should throw error for stateful mode', async () => {
    const messages: OpenAIMessage[] = [
      { role: 'user', content: 'Test stateful' },
    ];
