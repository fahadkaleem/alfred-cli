...
describe('OpenAIProvider.setModel', () => {
  ...
  it('should get the current model using getCurrentModel', () => {
    // Mock the getCurrentModel method on the provider
    const expectedModel = 'gpt-4';
    vi.spyOn(provider, 'getCurrentModel').mockReturnValue(expectedModel);
  ...
  it('should update model and retrieve it correctly', () => {
    ...
    // Mock getCurrentModel to return the new model
    vi.spyOn(provider, 'getCurrentModel').mockReturnValue(newModel);
