...
import type { AuthPrecedenceConfig, OAuthManager } from '../auth/precedence.js';
...
export abstract class BaseProvider implements IProvider {
  ...
  constructor(
    ...
    if (config.apiKey && config.apiKey.trim() !== '') {
      ...
      const providerSpecificKey = `${config.name}-auth-key`;
      settingsService.set(providerSpecificKey, config.apiKey);
    }
    ...
    const precedenceConfig: AuthPrecedenceConfig = {
      providerName: config.name, // Pass provider name for provider-specific key lookups
  ...
  setThrottleTracker(tracker: (waitTimeMs: number) => void): void {
    ...
    const logger = new DebugLogger('alfred:provider:base');
