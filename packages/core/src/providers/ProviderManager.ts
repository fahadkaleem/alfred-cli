...
export class ProviderManager implements IProviderManager {
  ...
  registerProvider(provider: IProvider): void {
    ...
    if (this.config?.getConversationLoggingEnabled()) {
      // Log each capability separately as the event expects individual capability properties
      const capabilityKeys: Array<keyof ProviderCapabilities> = [
        'supportsStreaming',
        'supportsTools',
        'supportsVision',
        'supportsPaidMode',
      ...
      for (const key of capabilityKeys) {
        ...
        if (typeof value === 'boolean') {
          logProviderCapability(
            this.config,
            new ProviderCapabilityEvent(provider['name'], key, value),
          );
        }
      }
    }
  ...
  setActiveProvider(name: string): void {
    ...
    if (
      ...
      const contextPreserved = this.isContextPreserved(
        previousProviderName,
        name,
      );
      const reason = contextPreserved
        ? 'Switching provider with context preserved'
        : 'Switching provider without context preservation';

      logProviderSwitch(
        this.config,
        new ProviderSwitchEvent(previousProviderName, name, reason),
      );
    }
