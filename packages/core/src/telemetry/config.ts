...
export async function resolveTelemetrySettings(options: {
  ...
  const enabled =
    argv.telemetry ??
    parseBooleanEnvFlag(env['ALFRED_TELEMETRY_ENABLED']) ??
    settings.enabled;

  const rawTarget =
    (argv.telemetryTarget as string | TelemetryTarget | undefined) ??
    env['ALFRED_TELEMETRY_TARGET'] ??
  ...
  const otlpEndpoint =
    ...
    env['ALFRED_TELEMETRY_OTLP_ENDPOINT'] ??
    env['OTEL_EXPORTER_OTLP_ENDPOINT'] ??
    settings.otlpEndpoint;

  const rawProtocol =
    (argv.telemetryOtlpProtocol as string | undefined) ??
    env['ALFRED_TELEMETRY_OTLP_PROTOCOL'] ??
  ...
  const logPrompts =
    ...
    parseBooleanEnvFlag(env['ALFRED_TELEMETRY_LOG_PROMPTS']) ??
    settings.logPrompts;

  const outfile =
    argv.telemetryOutfile ??
    env['ALFRED_TELEMETRY_OUTFILE'] ??
    settings.outfile;

  const useCollector =
    parseBooleanEnvFlag(env['ALFRED_TELEMETRY_USE_COLLECTOR']) ??
