...
2. User Settings (`~/.alfred/settings.json`)
3. Workspace Settings (`<project>/.alfred/settings.json`)
...
- **User `settings.json` (`~/.alfred/settings.json`):**
...
- **Workspace `settings.json` (`<project>/.alfred/settings.json`):**
...
Alfred CLI loads `settings.json` files from three levels: System, Workspace, and User. When it comes to the `mcpServers` object, these configurations are **merged**:
...
In corporate environments with strict network policies, you can configure Alfred CLI to route all outbound traffic through a corporate proxy. This can be set via an environment variable, but it can also be enforced for custom tools via the `mcpServers` configuration.
...
For auditing and monitoring purposes, you can configure Alfred CLI to send telemetry data to a central location. This allows you to track tool usage and other events. For more information, see the [telemetry documentation](../telemetry.md).
...
Here is an example of a system `settings.json` file that combines several of the patterns discussed above to create a secure, controlled environment for Alfred CLI.
