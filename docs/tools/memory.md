...
This document describes the `save_memory` tool for the Alfred CLI.

## Description

Use `save_memory` to save and recall information across your Alfred CLI sessions. With `save_memory`, you can direct the CLI to remember key details across sessions, providing personalized and directed assistance.
...
## How to use `save_memory` with the Alfred CLI

The tool appends the provided `fact` to a special `ALFRED.md` file located in the user's home directory (`~/.alfred/ALFRED.md`). This file can be configured to have a different name.
